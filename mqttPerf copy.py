...
NUM_SUBSCRIBERS = 3000  # 订阅用户数
NUM_PUBLISHERS =200  # 发布用户数
NUM_HEARTBEATS =20
SUB_TOPIC = [
...
def start_mqtt_clients():
    ...
    try:
        ...
        # 订阅耗时
        print(f"发送的消息数量：{len(publish_msg_count)}，接收到的消息数量：{len(receive_msg_count)}")
        calculate_sub_spendtime()
        for k in sub_spend_time_list:
            print(k)
        for j in receive_msg_spend_time_list:
            print(j)
        # 等待所有线程结束

...
def get_client_id(client):
    """安全地获取MQTT客户端ID"""
    if hasattr(client, 'client_id'):  # 先尝试使用公共属性
        return client.client_id.decode('utf-8') if isinstance(client.client_id, bytes) else client.client_id
    elif hasattr(client, '_client_id'):  # 备选方案
...
class MQTTLoadTester:
    ...
    def __init__(self, config_file=None):
        ...
        self.connections_count = (
                self.config.get("num_subscribers", 0) +
                self.config.get("num_publishers", 0) +
    ...
    def create_mqtt_client(self, client_id, subscribe=False):
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f"连接失败: {e}. 请确认MQTT代理 {self.config['broker']}: {self.config['port']} 正在运行。")
                return None
        except Exception as e:
    ...
    def connect_client(self, client, subscribe=False):
        ...
        try:
            ...
            # 记录连接时间
            # 获取用户数据的正确方式
            userdata = client.user_data_get() if hasattr(client, 'user_data_get') else {}

            # 如果userdata是一个字典，则更新连接时间和计数
            if isinstance(userdata, dict):
                userdata["connection_time"] = time.time()
                if "connection_count" in userdata:
                    userdata["connection_count"] += 1
                if "client_id" in userdata:
            ...
            if subscribe:
                ...
                for topic in self.config["sub_topics"]:
                    topic = client_id + topic
                    client.subscribe(topic, qos=self.config["qos_level"])
                    self.logger.info(f"==========>{client_id} 订阅主题：{topic}")
        except Exception as e:
            client_id = get_client_id(client)
            self.logger.error(f"MQTT 客户端 {client_id} 连接失败: {e}")
            ...
            try:
                ...
                with receive_msg_lock:
                    if message_id not in self.receive_msg_count:
                        ...
                        print(
                            f"收到新消息! ID: {message_id}, 延迟: {latency_ms: .2f}ms, 当前接收总数: {len(self.receive_msg_count)}")
    ...
    def on_publish(self, client, userdata, mid):
        ...
        try:
            ...
            if userdata and isinstance(userdata, dict):
                client_id = userdata.get('client_id', 'unknown')
            self.logger.error(f"处理发布回调时出错: 客户端={client_id}, 错误={e}")
    ...
    def generate_final_report(self):
        ...
        # 连接耗时
        self.conn_time_std_dev, self.avg_connect_time, self.min_connect_time, self.max_connect_time = self.calculate_squared_diffs(
            self.connect_elapsed_time)

        self.logger.info(
            f"平均连接速度：{self.avg_connect_time} ms，最快连接时间：{self.min_connect_time} ms，最慢连接时间：{self.max_connect_time} ms， "
            f"连接标准差：{self.conn_time_std_dev}"
        )
    ...
    def generate_excel_report(self):
        ...
        try:
            ...
            if connect_times:
                ...
                conn_time_variance = sum((x - avg_connect_time) ** 2 for x in connect_times) / len(
                    connect_times) if len(connect_times) > 1 else 0
                conn_time_std_dev = conn_time_variance ** 0.5

                conn_data = pd.DataFrame({
                    "连接指标": ["最小连接时间", "最大连接时间", "平均连接时间", "连接标准差", "总连接尝试次数",
                                 "成功连接次数"],
                    "值": [
        ...
        try:
            ...
            if self.connect_elapsed_time:
                if len(self.connect_elapsed_time) > 1:
                    conn_time_variance = sum((x - self.avg_connect_time) ** 2 for x in self.connect_elapsed_time) / len(
                        self.connect_elapsed_time)
                    conn_time_std_dev = conn_time_variance ** 0.5
            ...
            for topic, latencies in self.receive_msg_spend_time.items():
                if latencies:
                    ...
                    # 计算方差和标准差
                    variance = sum((x - avg_latency) ** 2 for x in latencies) / len(latencies) if len(
                        latencies) > 1 else 0
                    std_dev = variance ** 0.5
    ...
    def generate_report(self):
        ...
        try:
            ...
            print(
                f"成功连接率: {(len(self.connect_elapsed_time) - self.connection_failures) / max(1, len(self.connect_elapsed_time)) * 100: .2f}%")
    ...
    def print_performance_summary(self):
        ...
        try:
            ...
            # 计算消息丢失率
            loss_rate = ((len(self.publish_msg_count) - len(self.receive_msg_count)) / len(
                self.publish_msg_count) * 100) if len(self.publish_msg_count) > 0 else 0
            ...
            conn_success_rate = (len(self.connect_elapsed_time) / (
                    len(self.connect_elapsed_time) + self.connection_failures) * 100) if (
                                                                                                 len(self.connect_elapsed_time) + self.connection_failures) > 0 else 0
